...
struct MILPRelaxation <: AbstractFormulation
    ...
    A_leq::SparseMatrixCSC{Float64,Int64}
    b_leq::Vector{Float64}
...
"""
    MILPRelaxation(function_data, milp_variable_indices, eq_constraint_data, leq_constraint_data, f_min, f_max)
...
function MILPRelaxation(
    ...
    lower_bounds::Vector{Float64} = zeros(num_variables)
    upper_bounds::Vector{Float64} = ones(num_variables)
    ...
    return MILPRelaxation(
        A_eq,
        b_eq,
        eq_constraint_data.num_constraints,
        A_leq,
        b_leq,
        leq_constraint_data.num_constraints,
...
function add_vertex_constraints!(
    ...
    for c in [1, 2]  # c is the coordinate index (1 for x, 2 for y).
        ...
        add_rhs!(constraint_data, row, float(secant_vertices[1][c]))
