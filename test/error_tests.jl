@testset "test for errors" begin
    ...
    @test_throws(logger, ErrorException, construct_milp_relaxation(x->tan(x), [0.0, pi/2]))
    @test_throws(logger, ErrorException, construct_milp_relaxation(x->sin(x), [0.0, 2*pi]))
    @test_throws(logger, ErrorException, construct_milp_relaxation(x->x^3, [0.0, -1.0, 1.0]))
